...
sp_squared_2 = ((n-1)*var_X + (n-1)*var_Y)  / (n + n - 2);
t_2 = tinv(1-alpha/2, n+n-2);

ci_parametric_2(:, 1) = (xbar - ybar) - t_2 * sqrt(sp_squared_2) * sqrt(1/n + 1/n); 
ci_parametric_2(:, 2) = (xbar - ybar) + t_2 * sqrt(sp_squared_2) * sqrt(1/n + 1/n);

bootstrap_samples_X_2 = zeros(n, M, B);
bootstrap_samples_Y_2 = zeros(n, M, B);
...
for i=1:M
    for j=1:B
        idX = randi([1 n], n, 1);
        idY = randi([1 m], m, 1);
        bootstrap_samples_X_3(:, i, j) = X(idX, i);
        bootstrap_samples_Y_3(:, i, j) = Y(idY, i);
